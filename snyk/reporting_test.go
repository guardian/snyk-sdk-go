...
func TestReporting_ListIssues(t *testing.T) {
	setup()
	defer teardown()

	mux.HandleFunc("/v1/reporting/issues/", func(w http.ResponseWriter, r *http.Request) {
...
`)
	...
	response, _, err := client.Reporting.ListIssues(ctx, "test-id", from, to, req)
	if err != nil {
